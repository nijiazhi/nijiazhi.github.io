...
根据**Basic Exact Greedy Algorithm**算法，我们可以发现模型对特征中值的范围不敏感，只对顺序敏感。举个栗子，假设一个样本集合中某特征出现的值为**1, 4, 6, 7**， 那么把它换为**1, 2, 3, 4**，生成的树的结构是完全一样的，只不过是对应的判断条件改变了，比如把**小于6**换成了**小于3**而已。这也给我们一个启示，我们完全可以用**百分比**作为基础来构造模型。

![Approximate-Algorithm-for-Split-Finding](/assets/images/blog/xgboost/Approximate-Algorithm-for-Split-Finding.png)

我们用$D_k = {(x_1k,h_1), (x_2k,h2),(x_3k,h_3), \cdots, (x_nk,h_n)}$代表每个样本的第k个特征和其对应的二阶梯度所组成的集合。那么就能用百分比来定义下面这个排名函数$r_k: \boldsymbol{R} \to [0,+\infty)$
...
上式表示的就是该特征的值小于z的样本所占总样本的比例，于是就能用下面这个不等式来寻找分离点${s_k1, s_k2, \cdots, s_kl}$：
...
上式中$\epsilon$表示一个近似比例，或者叫做扫描步长。从最小值开始，每次增加$\epsilon * (\max\limits_i \boldsymbol{x_{ik}} - \min\limits_i \boldsymbol{x_{ik}})$作为分离点，然后在这些分离点中选择一个最大分数作为最后的分离点。

很直观的可以想到$D_k$的选择（或者说$\max\limits_i \boldsymbol{x_{ik}} - \min\limits_i \boldsymbol{x_{ik}}$）有两种：

1. 全局选择：开始选好，然后每次分离都不变，即在总样本中选最大最小值；
2. 局部选择：每次分离后在分离的样本中选，即在前文所定义的叶子节点样本$I_j$中选。

可以看出，局部选择相对繁琐些，但效果会比方法1好些。

最后，这几种Split-Finding方法的实验比较，可以看到局部选择比全局选择优秀的多。另外，近似算法几乎和贪婪算法差不多。
