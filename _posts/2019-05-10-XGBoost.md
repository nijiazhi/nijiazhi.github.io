...
$$
\begin{aligned}
    Obj^{(t)} & \approx \sum\limits_{i=1}^{n} [L(y_i, \widehat{y_i}^{(t-1)}) + g_i f_t(x_i) + \frac{1}{2} h_i f_t^2(x_i)] + \Omega(f_t) + constant \\
              & = \sum\limits_{i=1}^{n} [g_i f_t(x_i) + \frac{1}{2} h_i f_t^2(x_i)] + f_t(x_i)) + \Omega(f_t) + [\sum\limits_{i=1}^{n} L(y_i, \widehat{y_i}^{(t-1)}) + constant]
\end{aligned}
$$
