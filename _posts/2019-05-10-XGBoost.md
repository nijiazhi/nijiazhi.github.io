...
$$
\begin{cases}
    g_i = f^{(\prime)}(x) = \frac{\partial L(y_i, \quad \widehat{y_i}^{(t-1)})}{\partial \widehat{y_i}^{(t-1)}} \\
    \ \\
    h_i = f^{(\prime\prime)}(x) = \frac{\partial^{2} L(y_i, \quad \widehat{y_i}^{(t-1)})}{\partial({\widehat{y_i}^{(t-1)}})^{2}}
\end{cases}
$$
...
\begin{aligned}
    ...
    & = \sum\limits_{i=1}^{n} 
        ...
        \begin{matrix} 
            \underbrace{ \frac{\partial L(y_i, \quad \widehat{y_i}^{(t-1)})}{\partial \widehat{y_i}^{(t-1)}} } \\ {g_i}
        \end{matrix}
        \begin{matrix} \underbrace{ f_t(x_i) } \\ {\Delta{x}} \end{matrix} + 
        \frac{1}{2} 
        \begin{matrix} \underbrace{ \frac{\partial^{2} L(y_i, \quad \widehat{y_i}^{(t-1)})}{\partial({\widehat{y_i}^{(t-1)}})^{2}} } \\ {h_i} \end{matrix} 
        \begin{matrix} \underbrace{ f_t^2(x_i) } \\ {(\Delta{x})^2} \end{matrix}
        \right ] + \Omega(f_t) + constant \\
    & \approx \sum\limits_{i=1}^{n} \left [L(y_i, \widehat{y_i}^{(t-1)}) + g_i f_t(x_i) + \frac{1}{2} h_i f_t^2(x_i) \right ] + \Omega(f_t) + constant \\
