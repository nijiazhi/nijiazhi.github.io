...
#### Basic Exact Greedy Algorithm For SPLIT FINDING

从树的深度为0开始，每一个节点暴力遍历所有特征。对每个特征，先按照该特征里的值进行**预排序**，然后线性扫描来决定最好的分割点，最后在所有特征里选择分割后，Gain最高的那个特征。



这时，就有两种后续方法：

- 1. 最好的分割情况下，Gain为负时就停止树的生长，这样效率比较高也简单，但也放弃了未来可能会有更好的情况。
