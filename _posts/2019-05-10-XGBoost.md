...
从树的深度为0开始，每一个节点遍历所有特征。对每个特征，先按照该特征里的值进行**预排序**，然后线性扫描来决定最好的分割点，最后在所有特征里选择分割后，Gain最高的那个特征。
...
\begin{aligned}
    ...
    Gain &= Obj_{nosplit} - Obj_{split}
         &= -\frac{1}{2} \left[ \frac{G_L^2}{H_L + \lambda}  + \frac{G_R^2}{H_R + \lambda} \right - \frac{{(G_L + G_R)}^2}{H_L + H_R + \lambda} \right ] - \gamma (T_split - T_nosplit)
\end{aligned}
$$
