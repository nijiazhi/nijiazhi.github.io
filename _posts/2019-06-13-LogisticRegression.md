...
其实为某种形式的回归建立数学模型并不是一件容易的事情，经过先烈的曲折探索，得出了一个神奇的公式
...
而$x^\mathrm{T} \Delta w$的值不就是反映感知机模型的输出嘛！（即$x^\mathrm{T} \Delta w > 0$则预测类别为正，$x^\mathrm{T} \Delta w < 0$则预测类别为负），我们再把$x^\mathrm{T} \Delta w$整理的好看一点，变成更正常的形式：$wx+b$，然后就可以得到下面的结论：
